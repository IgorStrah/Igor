...
#define CandleMinutes 100// how long the candle burns (in minutes)
#define NightLevel 15  // darkness level for night (lower = darker)  30 000 and >
#define DayLevel 10  // darkness level for day   (lower = brighter) 20 000 and <
...
#define MeasurementInterval 2 // delay between two light measurements (units of 4s. 30 = 2minutes)
 ...
 // clock_prescale_set(clock_div_16);  //slow down to 500kHz
  ...
  //      WDTCR |= (1<<WDP3); // set prescale timer to 4 s
        ...
        if (mode)
        {
          ...
          if (whonowdaynight(NightLevel)==0)  counter++;
          //if (isDarkerThan(NightLevel))  counter++;
          else if (counter) counter--;
        }
        else
        {
          if (whonowdaynight(DayLevel)==1)  counter++;
...
uint16_t whonowdaynight(uint16_t level)
{
   long int j;
...
if (whonowdaynight(NightLevel)==0)  
{
 PORTB |=(1<<LED1);  
  PORTB &= ~(1<<LED2);  
}
 else   if (whonowdaynight(DayLevel)==1)  
    {
 PORTB &= ~(1<<LED1);  
 PORTB|=(1<<LED2);  
    }
