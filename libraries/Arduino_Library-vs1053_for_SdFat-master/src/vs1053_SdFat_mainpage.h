...
- Software
  ...
  - The most current <A HREF = "https://github.com/mpflaga/Sparkfun-MP3-Player-Shield-Arduino-Library.git" > vs1053 Driver </A>
...
\subsection Install_Manager IDE Library Manager
 (1.3.+ and later)
 ...
 \subsection Install_Options Optional libraries
 ...
 - TimerOne
  ...
  - or can be found at http://playground.arduino.cc/Code/Timer1
 ...
 \subsection Install_Setup Initial Setup

 -# After correpsonding libraries are installed.
 -# Copy the contents of this projects the '.\Arduino\libraries\vs1053_for_SdFat\plugins\.' folder onto the root of the SdCard to be used. Along with desired audio files named appropriately; track001.mp3 through track009.mp3.
...
As mentioned the initial and principal support of this library is with Arduino 328 UNO/Duemilanove with a SparkFun MP3 Player Shield. Although various other boards and shields may be implemented by customing the \ref vs1053Config.h file.

\subsection ArduinoBareTouch Arduino Bare Touch
Support for Bare Conductive's Touch Board is provided and documented in \ref vs1053Config.h.

\note Reference \ref Arduino_Leonardo's note about example files.

\subsection ArduinoMega Arduino Mega Board
Support for Arduino and Seeeduino Mega's are documented in \ref vs1053Config.h, which simply REQUIRES additional jumpers. As the SPI are not on the same pins as the UNO/Duemilanove.

\subsection Arduino_Leonardo Arduino Leonardo Board
Support for Arduino Leonardo's are afflicted by having the SPI and INT0 pins not routed to the same pins as the UNO/Duemilanove . This is similar to the Arduino Mega. Which simply REQUIRES additional jumpers, as documented in \ref vs1053Config.h to correct the SPI. The swapping of INT0/INT1 is automatically corrected based on the Leonardo's processor type of __AVR_ATmega32U4__ being detected.

\note While the Leonardo has the same amount of physical program space as the UNO, it actually has less space available for use. As it uses approximately 4K, for core library USB features. Where there is adaquate space available for typical applications, the provided example files VS1053_Library_Demo.ino and FilePlayer.ino will compile a reduced set of examples based on the processor type of __AVR_ATmega32U4__ being detected. This is only for the demonstration, and other sketchs may call any of these functions, given its environment.
...
- <b>Non-Blocking:</b>
The controlling sketch needs to enquire via vs1053::isPlaying as to determine if the current audio stream is finished or still playing. This is actually good and a result of the library being non-blocking, allowing the calling sketch to simply initiate the play of a desired audio stream from SdCard by simply calling playTrack or playMP3, of the desired file, and move on with other RealTime issues.

- <b>Multi-Chip VS10xx support:</b>
Not at this time. There are too many issues with member functions of the vs1053 class requiring to be static.

- <b>Audio Input</b>
Most commericially available shields at this time do not support either Line Level or Microphone Input. With the exception of the Seeeduino MP3 Shield and other home made shields. Where as the below admx____.053 and vs1053::ADMixerLoad and vs1053::ADMixerVol are provided for such devices. Otherwise the example VS1053_Library_Demo.ino has these lines commented out in setup(). As to reduce complications. To re-enable simply uncomment.
...
\em vs_plg_to_bin.pl is a perl script, that is provided in this library to run on your PC, to read and digest the .plg files converting them to raw binary as to be read by vs1053::VSLoadUserCode() from the SdCard.
...
\note \b patches.053 is a cumulative update correcting many known troublesome issues. Hence patches.053 is attempted in vs1053::vs_init.
...
\warning SdFatLib initially only supports 8.3 filenames. Long file names have not been implemented.
